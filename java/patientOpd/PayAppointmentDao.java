...
public class PayAppointmentDao {
	public int calculateDaysBetweenAppointments(int bookedId, Connection connection) {
	    ...
	    try (PreparedStatement statement = connection.prepareStatement(
	            "SELECT patient_id, doctor_id, date FROM opd_appointments WHERE booked_id = ?")) {
	        ...
	        try (ResultSet resultSet = statement.executeQuery()) {
	            if (resultSet.next()) {
	                ...
	                Timestamp currentAppointmentTimestamp = resultSet.getTimestamp("date");
	                ...
	                try (PreparedStatement previousAppointmentStatement = connection.prepareStatement(
	                        "SELECT date FROM opd_appointments WHERE patient_id = ? AND doctor_id = ? AND booked_id != ? AND date < ? ORDER BY date DESC LIMIT 1")) {
	                    ...
	                    previousAppointmentStatement.setTimestamp(4, currentAppointmentTimestamp);

	                    try (ResultSet previousAppointmentResultSet = previousAppointmentStatement.executeQuery()) {
	                        if (previousAppointmentResultSet.next()) {
	                            Timestamp previousAppointmentTimestamp = previousAppointmentResultSet.getTimestamp("date");

	                            // Calculate days between appointments
	                            long millisecondsBetween = currentAppointmentTimestamp.getTime()
	                                    - previousAppointmentTimestamp.getTime();
	...
	public float getDoctorRate(int bookedId, Connection connection) {
		...
		try {
			String query = "SELECT dr.rate " + "FROM doctor_rates dr "
					+ "JOIN opd_appointments ba ON dr.doctor_id = ba.doctor_id " + "WHERE ba.booked_id = ?";
			try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
