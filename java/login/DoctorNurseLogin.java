	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			ps = con.prepareStatement(
					"select * from employeelist where username=? and password=?");
			ps.setString(1, username);
			ps.setString(2, password);
			rs = ps.executeQuery();
			if (rs.next()) 
			{
				session.setAttribute("id", rs.getInt("employee_id"));
				...
				}else if( rs.getString("role").equals("nurse") )
				{
					...
					.forward(request, response);
				}else if( rs.getString("role").equals("lab") )
				{
					request.getRequestDispatcher("lab/lab_dashboard.jsp")
					.forward(request, response);
				}else if( rs.getString("role").equals("pharma") )
				{
					request.getRequestDispatcher("pharma/pharma_dashboard.jsp")
					.forward(request, response);
				}
				else {
