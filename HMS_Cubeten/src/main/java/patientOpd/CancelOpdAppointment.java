...
public class CancelOpdAppointment extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String appointmentQuery = "SELECT * FROM opd_appointments WHERE booked_id = ?";
			try (PreparedStatement appointmentStatement = connection.prepareStatement(appointmentQuery)) {
				...
				try (ResultSet appointmentResultSet = appointmentStatement.executeQuery()) {
					if (appointmentResultSet.next()) {
						...
						request.setAttribute("date", appointmentResultSet.getTimestamp("date"));

						// Add other attributes as needed
					}
				} // end of try-with-resources appointmentResultSet
			} // end of try-with-resources appointmentStatement
			Object dateAttribute = request.getAttribute("date");
			if (dateAttribute != null) {
				System.out.println("Date Attribute Type: " + dateAttribute.getClass().getName());
			} else {
				System.out.println("Date Attribute is null");
			}

			// Fetch patient name from opd_patient
			String patientQuery = "SELECT fullname FROM patients WHERE patientid = ?";
			try (PreparedStatement patientStatement = connection.prepareStatement(patientQuery)) {
				...
				try (ResultSet patientResultSet = patientStatement.executeQuery()) {
					if (patientResultSet.next()) {
						System.out.println(patientResultSet.getString("fullname"));
						request.setAttribute("patient_name", patientResultSet.getString("fullname"));
					}
			...
			String departmentQuery = "SELECT dpt_name FROM department WHERE department_id = ?";
			try (PreparedStatement departmentStatement = connection.prepareStatement(departmentQuery)) {
				...
				try (ResultSet departmentResultSet = departmentStatement.executeQuery()) {
					if (departmentResultSet.next()) {
						request.setAttribute("department_name", departmentResultSet.getString("dpt_name"));
					}
			...
			String doctorQuery = "SELECT firstname , lastname FROM employeelist WHERE employee_id = ?";
			try (PreparedStatement doctorStatement = connection.prepareStatement(doctorQuery)) {
				doctorStatement.setInt(1, dcid);
				try (ResultSet doctorResultSet = doctorStatement.executeQuery()) {
					if (doctorResultSet.next()) {
						request.setAttribute("doctor_name", doctorResultSet.getString("firstname")+" "+doctorResultSet.getString("lastname"));
					}
				} // end of try-with-resources doctorResultSet
			} // end of try-with-resources doctorStatement

			// Forward to cancel_receipt.jsp
			RequestDispatcher dispatcher = request.getRequestDispatcher("patient/cancel_receipt.jsp");
	 ...
	 */
	private float getAmountFromOpdBill(int booked_id, Connection connection) throws SQLException {
		float amount = 0.0f;
	 ...
	 */
	private void updateAppointmentStatus(int booked_id, Connection connection) throws SQLException {
		
		 // Query to update appointment_status to cancelled 
		String updateStatusQuery =
		  "UPDATE opd_appointments SET appointment_status = 'cancelled' WHERE booked_id = ?";  
		...
		// Query to update appointment_status to cancelled and delete the record



	/**
	 * Insert into opd_bill table new record with negative amount, pay status to
	 * refunded
	 */
	private void insertRefundRecord(int booked_id, double amount, Connection connection) throws SQLException {
		// Query to insert a new record into opd_bill
		String insertRefundQuery = "INSERT INTO opd_bill (booked_id, pay_status, date, amount) VALUES (?, ?, NOW(), ?)";
		try (PreparedStatement insertRefundStatement = connection.prepareStatement(insertRefundQuery)) {
