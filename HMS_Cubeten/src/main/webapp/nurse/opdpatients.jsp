					...
					<table class="table table-striped" id="example">
						...
						<tbody>
							...
							try {
								...
								String sqlQuery = "SELECT " + "oa.booked_id, " + "oa.patient_id, " + "p.fullname AS patient_fullname, " + "p.age, "
								+ "p.gender, " + "oa.date, " + "oa.appointment_status, " + "d.dpt_name AS department_name, "
								+ "e.firstname AS doctor_firstname, " + "e.lastname AS doctor_lastname " + "FROM " + "opd_appointments oa "
								+ "JOIN " + "patients p ON oa.patient_id = p.patientid " + "JOIN "
								+ "department d ON oa.department_id = d.department_id " + "JOIN "
								+ "employeelist e ON oa.doctor_id = e.employee_id WHERE date >= ? AND date <= ?";
							...
							<tr>
								...
								<td>
									<form action="/HMS_Cubeten/nurse/n_acknowledge.jsp" method="post">
										<input type="hidden" name="booked_id1"
											value="<%=rs.getInt("oa.booked_id")%>"> <input
											type="hidden" name="age" value="<%=rs.getInt("p.age")%>">
										<input type="hidden" name="gender"
											value="<%=rs.getString("p.gender")%>"> <input
											type="hidden" name="patient_id1"
											value="<%=rs.getString("patient_fullname")%>"> <input
											type="hidden" name="doctor_id1"
